...
namespace Darkorbit.Game.GalaxyGates
{
    internal class BetaGates
    {
        ...
        public BetaGates(int map, GameSession gameSession, int gateWave, int gateLives)
        {
        ...
        public async void Start()
        {
            ...
            if (!currentPlayer.Destroyed)
            {
                ...
                {
                    mySqlClient.ExecuteNonQuery($"UPDATE player_galaxygates SET wave = '{wave}' WHERE userId = {currentPlayer.GetPlayerId()} AND gateId = '2'");
                }
            }
        }
        ...
        public void CreateNewWaveSub(int npc, int count, int health, int wave, bool lastSubwave)
        {
            ...
            currentPlayer.SendPacket($"0|A|STD|Map Beta - Wave {wave}");
            if (!lastSubwave)
            {
                if (currentPlayer.positionInitializacion.mapID != 52)
                {
                    return;
                }
            }
        ...
        public void CreateNPC(int npc, int count, int health, int wave)
        {
            for (int i = 0; i < count; i++)
            {
                ...
                int npcNumber = i + 1;
                var npcbeta = new NpcGG(Randoms.CreateRandomID(), GameManager.GetShip(npc), GameManager.GetSpacemap(gateMapId), Position.GetPosOnCircle(new Position(10500, 6500), 5000), "β", wave, currentPlayer, 52, gateMapId); // Create NPC
            }
        }
        ...
        public async void WaveCheck(Player player)
        {
            ...
            if (enemysCount == 0)
            {
                ...
                else // Gate Done
                {
                    ...
                    currentPlayer.AmmunitionManager.AddAmmo(Objects.Players.Managers.AmmunitionManager.UCB_100, Randoms.random.Next(40000, 40000));
                    currentPlayer.AmmunitionManager.AddAmmo(Objects.Players.Managers.AmmunitionManager.RSB_75, Randoms.random.Next(3000, 5000));

                    // update database
                    using (SqlDatabaseClient mySqlClient = SqlDatabaseManager.GetClient())
                    {
                        mySqlClient.ExecuteNonQuery($"UPDATE player_galaxygates SET prepared = '{0}', lives = '{3}', parts = '[]', wave = '{1}' WHERE userId = {currentPlayer.GetPlayerId()} AND gateId = '2'");
                    }
        ...
        public void RemoveLife()
        {
            if (lives <= 1)
            {
                using (SqlDatabaseClient mySqlClient = SqlDatabaseManager.GetClient())
                {
                    mySqlClient.ExecuteNonQuery($"UPDATE player_galaxygates SET prepared = '{0}', lives = '{0}', wave = '{1}', parts = '[]' WHERE userId = {currentPlayer.GetPlayerId()} AND gateId = '2'");
                }
            }
            else
            {
                ...
                using (SqlDatabaseClient mySqlClient = SqlDatabaseManager.GetClient())
                {
                    mySqlClient.ExecuteNonQuery($"UPDATE player_galaxygates SET lives = '{newLives}' WHERE userId = {currentPlayer.GetPlayerId()} AND gateId = '2'");
                }
            }
        }
        ...
        {
            alphaGateNextWavePortal = new Portal(GameManager.GetSpacemap(player.GetPlayerActiveMap()), new Position(9500, 6500), new Position(10500, 6500), 52, 3, 3, true, true, false); // Next wave
