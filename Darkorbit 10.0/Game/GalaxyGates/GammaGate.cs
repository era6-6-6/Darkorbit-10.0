...
namespace Darkorbit.Game.GalaxyGates
{
    internal class GammaGates
    {
        ...
        public GammaGates(int map, GameSession gameSession, int gateWave, int gateLives)
        {
        ...
        public async void Start()
        {
            ...
            if (!currentPlayer.Destroyed)
            {
                ...
                {
                    mySqlClient.ExecuteNonQuery($"UPDATE player_galaxygates SET wave = '{wave}' WHERE userId = {currentPlayer.GetPlayerId()} AND gateId = '3'");
                }
            }
        }
        ...
        public void CreateNewWaveSub(int npc, int count, int health, int wave, bool lastSubwave)
        {
            ...
            currentPlayer.SendPacket($"0|A|STD|Map Gamma - Wave {wave}");
            if (!lastSubwave)
            {
                if (currentPlayer.positionInitializacion.mapID != 53)
                {
                    return;
                }
            }
        ...
        public void CreateNPC(int npc, int count, int health, int wave)
        {
            for (int i = 0; i < count; i++)
            {
                ...
                int npcNumber = i + 1;
                var npcgamma = new NpcGG(Randoms.CreateRandomID(), GameManager.GetShip(npc), GameManager.GetSpacemap(gateMapId), Position.GetPosOnCircle(new Position(10500, 6500), 5000), "γ", wave, currentPlayer, 53, gateMapId); // Create NPC
            }
        }
        ...
        public async void WaveCheck(Player player)
        {
            ...
            if (enemysCount == 0)
            {
                ...
                else // Gate Done
                {
                    ...
                    currentPlayer.AmmunitionManager.AddAmmo(Objects.Players.Managers.AmmunitionManager.UCB_100, Randoms.random.Next(60000, 60000));
                    currentPlayer.AmmunitionManager.AddAmmo(Objects.Players.Managers.AmmunitionManager.RSB_75, Randoms.random.Next(5000, 7500));

                    // update database
                    using (SqlDatabaseClient mySqlClient = SqlDatabaseManager.GetClient())
                    {
                        mySqlClient.ExecuteNonQuery($"UPDATE player_galaxygates SET prepared = '{0}', lives = '{3}', parts = '[]', wave = '{1}' WHERE userId = {currentPlayer.GetPlayerId()} AND gateId = '3'");
                    }
        ...
        public void RemoveLife()
        {
            if (lives <= 1)
            {
                using (SqlDatabaseClient mySqlClient = SqlDatabaseManager.GetClient())
                {
                    mySqlClient.ExecuteNonQuery($"UPDATE player_galaxygates SET prepared = '{0}', lives = '{0}', wave = '{1}', parts = '[]' WHERE userId = {currentPlayer.GetPlayerId()} AND gateId = '3'");
                }
            }
            else
            {
                ...
                using (SqlDatabaseClient mySqlClient = SqlDatabaseManager.GetClient())
                {
                    mySqlClient.ExecuteNonQuery($"UPDATE player_galaxygates SET lives = '{newLives}' WHERE userId = {currentPlayer.GetPlayerId()} AND gateId = '3'");
                }
            }
        }
        ...
        {
            alphaGateNextWavePortal = new Portal(GameManager.GetSpacemap(player.GetPlayerActiveMap()), new Position(9500, 6500), new Position(10500, 6500), 53, 4, 3, true, true, false); // Next wave
