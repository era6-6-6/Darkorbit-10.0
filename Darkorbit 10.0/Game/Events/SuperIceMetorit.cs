...
{
    internal class SuperIceMetorit
    {
        ...
        public static void Starting()
        {
            try
            {
                if (Active)
                {
                    ...
                    int seconds = 60;
                    int k = seconds;
                    GameManager.SendPacketToAll($"0|n|KSMSG|SUPER METEORIT WILL LAND IN MAP {Spacemap.Name} IN {seconds} SECONDS");
                    for (int i = seconds; i > 0; i--)
                    {
                        if (i == k)
                        {
                            string packet = $"0|A|STD|[Map {Spacemap.Name}] Super Meteorit land in {i} seconds...";
                    ...
                    GameManager.SendPacketToAll($"0|n|KSMSG|SUPER METEORIT LAND IN MAP {Spacemap.Name}");
                    CurrentPosition = new Position(new Random().Next(0, 20600), new Random().Next(0, 12600));
                    demaner = createNPC(33, 1, Spacemap.Id, CurrentPosition);
            ...
            {
                Logger.Log("error_log", $"- [SuperIceMeteorit.cs] Main void exception: {ex}");
            }
        }
            ...
            try
            {
                if (Active)
                {
                    while (Active)
                    {
                        ...
                        if (Active && Spawned)
                        {
                            GameManager.SendPacketToAll($"0|n|KSMSG|SUPER METEORIT IN MAP {Spacemap.Name} POSITION {demaner.Position.X / 100}/{demaner.Position.Y / 100}");
                        }
                ...
                while (true)
                {
                    ...
                    if (demaner != null)
                    {
                        if (demaner.Destroyed)
                        {
                            for (int i = 0; i < 10; i++)
                            {
                                new SuperIceBox(Position.Random(Spacemap, demaner.Position.X - 400, demaner.Position.X + 400, demaner.Position.Y - 400, demaner.Position.Y + 400), Spacemap, false);
                            }
        ...
        public static void sendReward()
        {
            ...
            GameManager.SendPacketToAll($"0|n|KSMSG|SUPER ICE METEORIT DESTROYED");
            ...
            if (EventManager.GetSuperIceMeteorid() == true)
            {
                if (!Active && !Spawned)
                {
