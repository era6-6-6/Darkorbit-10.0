...
namespace Darkorbit.Game.Objects
{
    class Hitac : Character
    {
        ...
        public Hitac(int id, int typeId) : base(id, GameManager.GetShip(typeId).Name, 0, GameManager.GetShip(typeId), CurrentPosition, GameManager.GetSpacemap(16), GameManager.GetClan(0), 22)
        {
            Speed = 100;
        }
        ...
        public void SendReward()
        {
            ...
            for (int i = 0; i <= 20; i++)
                new CargoBox(Position.Random(Spacemap, Position.X - 1000, Position.X + 500, Position.Y - 1000, Position.Y + 500), Spacemap, false, false, true);

            if (Mmo >= EventManager.Hitac.Limit || Eic >= EventManager.Hitac.Limit || Vru >= EventManager.Hitac.Limit)
