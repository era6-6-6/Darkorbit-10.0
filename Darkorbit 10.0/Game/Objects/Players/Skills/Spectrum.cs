namespace Darkorbit.Game.Objects.Players.Skills
{
    class Spectrum : Skill
    {
        ...
        {
            if (Ship.SPECTRUMS.Contains(Player.Ship.Id) && cooldown.AddMilliseconds(Duration + Cooldown) < DateTime.Now || Player.Storage.GodMode || Player.Ship.Id == Ship.SPECTRUM_TYRANNOS && cooldown.AddMilliseconds(Cooldown) < DateTime.Now || Player.Storage.GodMode || Player.Ship.Id == Ship.SPECTRUM_ARGON && cooldown.AddMilliseconds(Cooldown) < DateTime.Now || Player.Storage.GodMode)
            {
