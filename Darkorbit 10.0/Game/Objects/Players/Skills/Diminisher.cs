namespace Darkorbit.Game.Objects.Players.Skills
{
    class Diminisher : Skill
    {
        ...
        {
            if (Ship.DIMINISHERS.Contains(Player.Ship.Id) && cooldown.AddMilliseconds(Duration + Cooldown) < DateTime.Now || Player.Storage.GodMode || Player.Ship.Id == Ship.DIMINISHER_PHANTASM && cooldown.AddMilliseconds(Cooldown) < DateTime.Now || Player.Storage.GodMode || Player.Ship.Id == Ship.DIMINISHER_ULLRIN && cooldown.AddMilliseconds(Cooldown) < DateTime.Now || Player.Storage.GodMode)
            {
